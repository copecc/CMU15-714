...
def softmax_loss(Z, y):
    ...
    """
    ### BEGIN YOUR CODE
    return np.mean(-np.log(np.exp(Z) / np.sum(np.exp(Z), axis=1, keepdims=True))[np.arange(len(y)), y])
    ### END YOUR CODE
...
def softmax_regression_epoch(X, y, theta, lr=0.1, batch=100):
    ...
    for i in range(0, num_examples, batch):
        X_batch = X[i : i + batch]
        y_batch = y[i : i + batch]
        # loss = softmax_loss(X_batch @ theta, y_batch)
        Z = np.exp(X_batch @ theta)
        Z = Z / np.sum(Z, axis=1, keepdims=True)
        I_y = np.zeros_like(Z)
        I_y[np.arange(len(y_batch)), y_batch] = 1
        dtheta = X_batch.T @ (Z - I_y) / len(X_batch)
        theta -= lr * dtheta
    ### END YOUR CODE
...
def nn_epoch(X, y, W1, W2, lr=0.1, batch=100):
    ...
    for i in range(0, num_examples, batch):
        ...
        y_batch = y[i : i + batch]
        # Forward
        Z1 = np.maximum(X_batch @ W1, 0)  # (batch, hidden_dim)
        Z = np.exp(Z1 @ W2)  # (batch, num_classes)
        # Softmax
        Z = Z / np.sum(Z, axis=1, keepdims=True)
        # One-hot
        I_y = np.zeros_like(Z)
        I_y[np.arange(len(y_batch)), y_batch] = 1
        # Backward
        G2 = Z - I_y  # (batch, num_classes)
        G1 = (Z1 > 0).astype(float) * (G2 @ W2.T)  # (batch, hidden_dim)
        # Update
        dW1 = X_batch.T @ G1 / len(y_batch)
        dW2 = Z1.T @ G2 / len(y_batch)
        W1 -= lr * dW1
        W2 -= lr * dW2
    ### END YOUR CODE
